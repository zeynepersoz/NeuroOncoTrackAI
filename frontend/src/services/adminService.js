...
export async function deleteAdminUser(userId) {
  return await callAdmin(`/admin/users/${userId}/deactivate`, { method: 'POST' });
}
...
export async function revokeAdminSession(userId, sessionId) {
  ...
  return await callAdmin(`/admin/users/${userId}/sessions/${sessionId}/terminate`, { method: 'POST' });
}

export async function forceLogoutAdminUser(userId) {
  return await callAdmin(`/admin/users/${userId}/force-logout`, { method: 'POST' });
}
...
export async function getAdminOrganizationDetail(orgId) {
  return await callAdmin(`/admin/organizations/${orgId}`);
}

export async function deactivateAdminOrganization(orgId) {
  return await callAdmin(`/admin/organizations/${orgId}/deactivate`, { method: 'POST' });
}
...
export async function getAdminAuditLog({ page = 1, severity = '' } = {}) {
  const url = severity
    ? `/admin/audit-logs?page=${page}&severity=${encodeURIComponent(severity)}`
...
export async function getAdminAuditLogDetail(logId) {
  return await callAdmin(`/admin/audit-logs/${logId}`);
}
